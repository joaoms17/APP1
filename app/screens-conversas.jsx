// screens-conversas.jsx — WhatsApp inbox + conversation + AI lead extraction
// ctx = { t, lang, accent, push, pop, goTab }

function ConversasScreen({ ctx }) {
  const { t, accent } = ctx;
  return (
    <div style={{ padding: '54px 0 24px' }}>
      <div style={{ padding: '0 18px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
          <Serif size={32} weight={500}>{t('tab_conversas')}</Serif>
      ...
      <div style={{ padding: '0 8px' }}>
        {CONVERSAS.map((c) => (
          <div key={c.id} onClick={() => ctx.push('conversa', { id: c.id })} style={{
            display: 'flex', gap: 13, alignItems: 'center', padding: '12px 12px', borderRadius: 16,
            cursor: 'pointer', background: c.unread ? hexToRgba(PALETTE.sage, 0.07) : 'transparent',
          }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 3, gap: 8 }}>
                ...
                {c.unread > 0 && (
                  <span style={{ minWidth: 18, height: 18, borderRadius: 9, background: accent, color: '#FBF7F0', fontFamily: 'var(--sans)', fontSize: 11, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 5px' }}>{c.unread}</span>
                )}
              </div>
              {c.aiReady && (
                <div style={{ marginTop: 6 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontFamily: 'var(--sans)', fontSize: 11, color: PALETTE.terracottaDark, fontWeight: 500 }}>
                    <Eucalyptus size={10} stem={PALETTE.terracottaDark} leaf={PALETTE.sage} /> Lead pronta a criar
                  </span>
                </div>
              )}
            </div>
...
// Conversation detail
// ─────────────────────────────────────────────────────────────
function ConversaScreen({ ctx, params }) {
  const { t, lang, accent } = ctx;
  const c = CONVERSAS.find((x) => x.id === params.id) || CONVERSAS[0];
  const [showExtract, setShowExtract] = React.useState(false);
  const [reply, setReply] = React.useState('');
  const [sent, setSent] = React.useState([]);
  const hasAI = c.aiReady;

  const suggestedReply = lang === 'pt'
    ? 'Olá Ana, que alegria! Parabéns pelo casamento. Sim, temos disponibilidade para 12 de setembro. Para a noiva e 4 damas, sugiro uma prova de penteado no estúdio. Envio já uma proposta com todos os detalhes.'
    : 'Hi Ana, how lovely! Congratulations. Yes, we have availability on 12 September. For the bride and 4 bridesmaids I suggest a hair trial at the studio. I will send a full proposal shortly.';
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--paper)' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '18px 16px 12px' }}>
        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          <span style={{ fontFamily: 'var(--sans)', fontSize: 10.5, letterSpacing: '0.12em', textTransform: 'uppercase', color: PALETTE.inkSoft, background: hexToRgba(PALETTE.ink, 0.06), padding: '4px 12px', borderRadius: 50 }}>Hoje · 14:21</span>
        </div>
      ...
      <div style={{ flexShrink: 0, padding: '10px 14px 30px', background: 'var(--paper-card)', borderTop: '1px solid rgba(74,63,53,0.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{ flex: 1, background: 'var(--paper)', borderRadius: 50, padding: '11px 16px', border: '1px solid rgba(74,63,53,0.08)' }}>
          <span style={{ fontFamily: 'var(--sans)', fontWeight: 300, fontSize: 13.5, color: reply ? PALETTE.ink : PALETTE.inkSoft }}>{reply || t('escrever')}</span>
        </div>
        <button onClick={() => { if (reply) { setSent((s) => [...s, reply]); setReply(''); } }} style={{ width: 44, height: 44, borderRadius: '50%', background: accent, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
function Bubble({ m, accent }) {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: me ? 'flex-end' : 'flex-start', marginBottom: 8 }}>
      <div style={{
        maxWidth: '78%', padding: '10px 14px', borderRadius: me ? '18px 18px 4px 18px' : '18px 18px 18px 4px',
        background: me ? hexToRgba(accent, 0.14) : 'var(--paper-card)',
        border: me ? 'none' : '1px solid rgba(74,63,53,0.06)',
        boxShadow: me ? 'none' : '0 2px 8px rgba(74,63,53,0.04)',
      }}>
...
Object.assign(window, { ConversasScreen, ConversaScreen });
