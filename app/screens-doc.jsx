// screens-doc.jsx — AI-generated branded document preview (orçamento)
// ctx = { t, lang, accent, push, pop, goTab }

function DocScreen({ ctx, params }) {
  const { t, lang, accent } = ctx;
  const r = reservaById(params.reservaId) || RESERVAS[0];
  const [sent, setSent] = React.useState(false);

  const lineItems = r.servicos.map((s) => {
    const prices = { makeup: [r.tipo === 'Convidada' ? 95 : 320, 'Maquilhagem noiva + prova'], hair: [380, 'Penteado noiva + 4 damas'], photo: [950, 'Cobertura fotográfica · dia completo'], dj: [800, 'DJ · 6 horas + equipamento'], music: [600, 'Música ao vivo · cerimónia'], planning: [1500, 'Coordenação completa'], video: [700, 'Vídeo highlights'] };
    const en = { makeup: 'Bridal makeup + trial', hair: 'Bridal hair + 4 bridesmaids', photo: 'Photography · full day', dj: 'DJ · 6 hours + gear', music: 'Live music · ceremony', planning: 'Full coordination', video: 'Highlights video' };
    return { name: SERVICE_LABEL[lang][s], desc: lang === 'pt' ? prices[s][1] : en[s], price: prices[s][0] };
  });
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--paper-warm)' }}>
      <DetailHeader ctx={ctx} title={t('orcamento')} sub={r.name} chip={<AIBadge label={t('gerado_ia')} />} />
        ...
        <div style={{ background: 'var(--paper-card)', borderRadius: 14, boxShadow: '0 14px 40px rgba(74,63,53,0.12)', overflow: 'hidden', border: '1px solid rgba(74,63,53,0.05)' }}>
          ...
          <div style={{ padding: '24px 28px 28px' }}>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <TotalRow label="IVA 23%" value={iva} muted />
            ...
            <div style={{ marginTop: 22, padding: '14px 16px', background: 'var(--surface-sage)', borderRadius: 12, border: '1px solid rgba(147,160,126,0.45)' }}>
              <div style={{ fontFamily: 'var(--sans)', fontWeight: 300, fontSize: 12, color: PALETTE.ink, lineHeight: 1.6 }}>
                {lang === 'pt'
                  ? 'Sinal de 30% para confirmar a reserva. Restante até 7 dias antes do evento. Proposta válida por 14 dias.'
                  : '30% deposit to confirm the booking. Balance up to 7 days before the event. Valid for 14 days.'}
              </div>
            </div>
          </div>
        </div>
...
function TotalRow({ label, value, muted }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0' }}>
      <span style={{ fontFamily: 'var(--sans)', fontWeight: 300, fontSize: 13, color: muted ? PALETTE.inkSoft : PALETTE.ink }}>{label}</span>
      <span style={{ fontFamily: 'var(--sans)', fontWeight: 400, fontSize: 13.5, color: muted ? PALETTE.inkSoft : PALETTE.nearBlack }}>€ {value.toLocaleString('pt-PT')}</span>
    </div>
  );
}

Object.assign(window, { DocScreen });
