    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--paper)' }}>
      ...
      <div style={{ flexShrink: 0, padding: '12px 18px 30px', background: 'var(--paper-card)', borderTop: '1px solid rgba(74,63,53,0.07)', display: 'flex', gap: 12 }}>
        <Btn variant="solid" accent={accent} size="md" full icon="arrowR" onClick={() => ctx.openCreate('reserva', {
          ...
          onComplete: async (row, { update }) => {
            // re-link the lead's proposals to the new booking so the reserva shows what was sent
            for (const p of (ctx.proposals || []).filter(p => p.lead_id === l.id)) await update('proposals', p.id, { lead_id: row.id })
            await update('leads', l.id, { estado: 'ganho' })
            ctx.pop()
          },
        })}>{t('converter')}</Btn>
      </div>
