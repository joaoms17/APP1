...
export function Icon({ name, size = 22, color = 'currentColor', stroke = 1.6, style }) {
...
export function ServiceGlyph({ k, size = 14, color }) {
...
export function Eucalyptus({ size = 22, stem = PALETTE.terracotta, leaf = PALETTE.sage, style }) {
  return (
...
export function Label({ children, color = PALETTE.inkSoft, size = 11, style }) {
  return (
    <div style={{ fontFamily: 'var(--sans)', textTransform: 'uppercase', letterSpacing: '0.28em', fontWeight: 500, fontSize: size, color, ...style }}>
      {children}
    </div>
  )
}

export function Serif({ children, size = 26, color = PALETTE.nearBlack, weight = 500, style }) {
  return (
...
export function Avatar({ initials, color = PALETTE.terracotta, size = 44, ring = false, style }) {
  return (
    <div style={{
...
export function AvatarStack({ ids, size = 30, teamById }) {
...
export function Chip({ tone, children, dot = true, size = 12 }) {
  return (
    <span style={{
...
export function ServiceChip({ k, lang, SERVICE_LABEL: SL }) {
  return (
    <span style={{
...
export function Card({ children, onClick, style, sage = false, pad = 18 }) {
  return (
...
export function Btn({ children, onClick, variant = 'solid', accent, full = false, size = 'md', icon, style }) {
...
export function Rule({ dot = false, style }) {
  return (
...
export function AIBadge({ label = 'IA' }) {
  return (
    <span style={{
